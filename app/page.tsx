...
async function getData() {
  ...
  try {
    ...
    const res = await axios.get(`${process.env.URL_PUBLIC_API}/api/books`);
    if (res.status !== 200) {
  ...
  return (
    <Layouts>
      ...
      {Array.isArray(datas) ? (datas.map((data: any) => (
        <div className="p-0 md:p-4 sm:p-2" key={ data.code }>
          <div className="max-w max-w-sm rounded overflow-hidden shadow-xl mt-10 md:mt-0" >

            
            <Link href="/product">
              <Image src={data.img} width={500} height={500} className="w-full" alt="logo" />
            </Link>

            <div className="px-6 py-4">
              <div className="font-bold text-xl mb-2">{data.title}</div>
              <p className="text-gray-700 text-base text-justify">{data.description}</p>
            </div>
